  ...
  return (
    <section id="new-arrivals" className="py-16 md:py-24 lg:py-32 bg-background overflow-hidden">
      <div className="mx-auto max-w-[1440px] px-5 sm:px-6 lg:px-12 mb-8 md:mb-12">
        <div className="flex items-end justify-between">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={{ duration: 0.7 }}>
            <p className="text-xs font-semibold tracking-[0.25em] uppercase text-accent mb-2">{t.newArrivals.subtitle}</p>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-foreground">{t.newArrivals.title}</h2>
          </motion.div>
      ...
      <div className="md:hidden mt-4 flex items-center justify-center gap-2 text-muted">
        ...
        <span className="text-[11px] tracking-wider uppercase font-medium">{t.newArrivals.swipe}</span>
      </div>
    </section>
  )
}